...
def test_scan_universe_emits_required_table_fields():
    ...
    assert top['flow_score'] == round(top['core_score'] * 0.7 + top['supplemental_score'] * 0.3, 1)
    assert top['flow_label'] in {'A+', 'A', 'B', 'C', '-'}
    assert 'Core ' in top['main_signal']
    assert 'Supp ' in top['main_signal']
